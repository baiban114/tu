package com.run.tu.controller.common;

import com.alibaba.fastjson2.JSONObject;
import com.run.tu.auth.JwtConstants;
import com.run.tu.core.result.GlobalResult;
import com.run.tu.core.result.GlobalResultGenerator;
import com.run.tu.dto.LinkToImageUrlDTO;
import com.run.tu.dto.TokenUser;
import com.run.tu.service.TuFileService;
import com.run.tu.util.FileUtils;
import com.run.tu.util.SpringContextHolder;
...
public class UploadController {
    ...
    public GlobalResult<JSONObject> uploadPicture(@RequestParam(value = "file", required = false) MultipartFile multipartFile, @RequestParam(defaultValue = "1") Integer type, HttpServletRequest request) throws IOException {
        ...
        String fileUrl = tuFileService.getFileUrlByMd5(md5, tokenUser.getIdUser(), fileType);
        ...
        try {
            ...
            tuFileService.insertTuFile(fileUrl, savePath, md5, tokenUser.getIdUser(), multipartFile.getSize(), fileType);
            data.put("url", fileUrl);
        } catch (IOException e) {
            data.put("message", "上传失败!");
        }
    ...
    public GlobalResult linkToImageUrl(@RequestBody LinkToImageUrlDTO linkToImageUrlDTO, HttpServletRequest request) throws IOException {
        ...
        try (InputStream inputStream = conn.getInputStream()) {
            ...
            tuFileService.insertTuFile(fileUrl, savePath, md5, tokenUser.getIdUser(), getData.length, fileType);
            data.put("originalURL", url);
            data.put("url", fileUrl);
            return GlobalResultGenerator.genSuccessResult(data);
        } catch (IOException e) {
