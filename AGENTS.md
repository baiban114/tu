...
- **知识点关联图**：资源管理「知识点关联图」Tab；默认以选定知识点为中心展开关联（进入时无预选中心点）；中心点通过通用 [`KnowledgePointPickerDialog`](tu-web-ts/src/components/knowledge/KnowledgePointPickerDialog.vue) / `openKnowledgePointPicker()` 选择
- **侧栏视图 · 标签检索**：视图目录「标签检索」在主内容区按标签（label 规范化相等）检索被标记的块（nodeView）与章节（`metadata.sectionTags`，`local/ref-group/ref-child` key），分页（10/页）按 `updatedAt` 倒序；行展开以只读原格式内联预览（章节=该标题起至同级标题前）。后端 `GET /api/kbs/{kbId}/tags` + `/tagged-content`，见 `docs/workspace-views.md` §5。最后查看的标签按知识库缓存到 `localStorage`（`tu:tag-content-view:last-tag`），再次进入「标签检索」视图或切换知识库时自动恢复该标签并直接加载结果（缓存标签不存在于标签池时忽略）。
...
- **Page types**: `PageItem.pageType` is `document` (default), `mindmap`, or `x6board`. Canvas pages (`mindmap` / `x6board`) render in `HomeView` via dedicated `.content-canvas` (not `.content-scroll` / `TuEditorPage`); `CanvasPage.vue` fills the area with a single primary X6 embed in `PageContent.embeds` + `metadata.primaryEmbedId`. If tree `pageType` is missing, workspace infers canvas type from content. Utilities: `tu-web-ts/src/utils/boardPageContent.ts`. Shared canvas UI: `BoardCanvasShell.vue` + `X6BoardBlock.vue` (embedded via `X6BlockView`, full-page via `CanvasPage`). Document内 x6 embed 可通过 NodeView 工具栏「升级为思维导图页/画板页」提升为独立页。Mindmap 节点富文本模式：`X6NodeOverlay` 内嵌 `TuEditor`，通过属性面板「文字模式」切换，graphData 存 `textMode`/`richContent`。画板系统剪贴板粘贴（`X6Component.vue` 顶层实现，`CanvasPage` / `X6BlockView` 衍生自动继承）：无选中节点且内部剪贴板为空时，按内容类型分发——图片文件直接上传创建图片节点；URL 创建链接卡片节点（graphData 存 `linkUrl`/`linkDisplay`），属性面板「链接展示形式」复用 `LinkPresentationModeBar`（额外提供图片模式）；其余文本创建富文本节点（`textMode: 'rich'` + `richContent`）。画板节点样式可互转（矩形/圆角矩形/椭圆/菱形，属性面板「节点样式」）：因 X6 不支持改既有节点 shape，采用同 id 重建节点并从快照恢复连线，保留文字/颜色/尺寸。图片、链接卡片、思维导图节点不参与转换。画板节点组合（工具栏「组合/取消组合」）：实现为组合容器节点（`data.boardGroup=true`，DOM 标 `data-board-group`）+ X6 embed 父子关系；边框样式在属性面板「样式→组合边框」可选胶囊式（`tight`，默认，紧贴成员四方边界，transparent fill 保证空白区可点击）或高亮边框（`highlight`，虚线与浅色填充、更大内缩）；容器位置/尺寸始终自动收放到成员包围盒 + 预设内缩边距（`fitBoardGroupContainer`，member 拖动/改尺寸时实时重算）。单击组内成员在组合未选中时优先选中最外层组合（首次点击上收），组合已选中时再次单击组内成员钻取选中该成员本身，Alt+单击直接仅选成员（Ctrl/⌘+单击保留给多选）；拖动时按选区区分组合级/成员级操作——选中组合容器时拖动成员经 `node:change:position` delta + `root.translate({exclude})` 传播整组（其余成员与容器整体平移）；仅选中单个成员（非组合容器）时拖动只移动该成员并让边框随之收放（其余成员不动，`beginBoardGroupDrag` 以 `graph.isSelected(root)` 门控，移动时 `refitBoardGroupForMember` 收束边框）；删除容器=解散（成员保留），成员 <2 的组自动解散；复制/复制副本时容器展开含后代。
