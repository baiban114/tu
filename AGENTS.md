...
- **侧栏视图 · 标签检索**：视图目录「标签检索」在主内容区按标签（label 规范化相等）检索被标记的块（nodeView）与章节（`metadata.sectionTags`，`local/ref-group/ref-child` key），分页（10/页）按 `updatedAt` 倒序；行展开以只读原格式内联预览（章节=该标题起至同级标题前）。后端 `GET /api/kbs/{kbId}/tags` + `/tagged-content`，见 `docs/workspace-views.md` §5。
