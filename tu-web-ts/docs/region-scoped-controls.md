...
> 状态：**已实施**（文档页 `TuEditorPage` 顶栏含编辑操作；标签与知识点元数据在标题下 `PageKnowledgeContextBar`）
...
禁止将仅编辑类顶栏能力只放在可滚动正文中部；筛选类标签展示与页面元数据（标签、知识点前置等）一并放在标题下方元数据区，便于同一区域管理。
...
| **标题下行（page title meta）** | 页面元数据：标签、知识点前置等 | `PageKnowledgeContextBar`（内含 `PageTagsBar`） |
...
- **放置**：区域固定顶栏或 Header；只读模式若仍可用，筛选标签仍在标题下元数据区展示。
- **标签**：页面元数据标签与正文内可筛选标签统一在 `PageKnowledgeContextBar` 展开后的标签行（`PageTagsBar`）；点击可筛选标签切换「按标签查看」，页面标签可点右上角叉号删除。
...
- 结构：
  ...
  2. 页面标题
  3. `PageKnowledgeContextBar` — 文档元数据（标签 + 知识点前置等；默认可收起）
  4. 正文 — 可随滚动离开视口

新增 page 级编辑能力时，优先并入 `page-chrome`；标签展示与筛选优先并入标题下元数据区。
...
| 能力 | 锚点 | 文件 |
|------|------|------|
| 按标签查看 / 页面标签 | 标题下元数据区 | `PageKnowledgeContextBar.vue` + `PageTagsBar.vue` |
