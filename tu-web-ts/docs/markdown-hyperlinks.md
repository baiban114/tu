...
2. **`()` 建议菜单**：仅在**源文本 IR**（或未完成的 `[](query` / `[title](query`）内改动 **href** 后出现；光标在 `[]` 或纯路过不展开。自动保存对 IR 做快照折叠写入，不改动实时文档（避免源文本闪烁/空格被 trim）。↓ 进入列表；回车选中 → `[]` 写 PDF/资源 title，`()` 写 locator。离开链接后 IR 再折叠为渲染链。
