...
const catalog: Record<string, PageHelp> = {
  ...
  'home.document': {
    ...
    features: [
      ...
      '插入整个资源时需选择实体访问地址（默认第一条）；按地址类型插入 PDF 摘页 / 图片 / 资源卡片。PDF 摘页可设起止页与纵向比例裁剪；块头/工具栏「划选笔记」可在 PDF 上拖拽框选区域，松开后可「发布笔记」或「应用裁剪」。有 `resource:` 来源时笔记绑定资源实体（任意嵌入该资源整体或重叠片段处可见）；无来源时写入页面 annotations（以 fileId 锚定，PDF↔链接转换后重挂）。默认隐藏，块头「显示笔记」后按区域显示，开关随块持久化；「高度 100%」按当前缩放将块高度调为刚好完整显示全部内容。',
      ...
      '划选工具栏：链接、标注、标签、标记节选与设置依据；标记节选后记为「学习进行中」目标（顶栏芯片显示资源实体名）。「标记为」文案为「资源实体名>最近前置标题」（与标记节选默认标题层级一致），确认后追加为同级引用块。支持 Markdown 链接 `[文字](url)`：敲 `)`、粘贴，或光标离开完整源码时转为链接；光标进入已有链接时 IR 展开为源码，离开后折叠。折叠态单击不跳转（Ctrl/⌘+单击才跳转内部 `page:`/`resource:`）；含 `#page=` 的 resource 链接在标题旁显示页码限制文本（可附带 `&clip=`）。在 `[]` 内**开始改动文字后**才出现搜索建议（↓ 进入列表、回车填入；仅移动光标路过不展开）；用 `文档关键词>标题关键词` 可链到页内标题（`page:…:heading:…`）。资源侧定位 › 搜索：`资源名>` / `资源名>章名>` 浏览下一层；末段有关键字则在当前作用域子树内深度搜索（列表只显示命中节点名，选中写入完整路径 + `resource:…:chapter|excerpt:…`）；用 `-12` / `-12-20`（或末段纯数字 `资源>12-20`）附带与 PDF 相同的起止页，写入 `#page=`。链接展示（悬停工具栏 / PDF 块工具栏共用）：外链可切换链接·iframe·标题；`resource:` 可切换链接·PDF，互相往返且不登记外链。站内 PDF 页码参照 Obsidian：`[标题](resource:ri-…#page=12)` 或 `#page=12-20`；纵向裁剪用 `&clip=上-下`（0–1 ratio），如 `#page=12&clip=0.2-0.75`；与 PDF 摘页起止页、纵向 ratio 双向同步（改摘页/裁剪会回写片段，全文模式去掉片段）。',
