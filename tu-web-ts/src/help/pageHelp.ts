...
const catalog: Record<string, PageHelp> = {
  ...
  'home.document': {
    ...
    features: [
      ...
      '划选工具栏：链接、标注、标签、标记节选与设置依据；标记节选后记为「学习进行中」目标（顶栏芯片显示资源实体名）。「标记为」文案为「资源实体名>最近前置标题」（与标记节选默认标题层级一致），确认后追加为同级引用块。支持 Markdown 链接 `[文字](url)`：敲 `)`、粘贴，或光标离开完整源码时转为链接；光标进入已有链接时 IR 展开为源码，离开后折叠。在 `[]` 内可搜索文档/资源（↓ 进入列表、回车填入并覆盖显示文字）；用 `文档关键词>标题关键词` 可链到页内标题（`page:…:heading:…`）。',
