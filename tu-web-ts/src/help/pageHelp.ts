...
const catalog: Record<string, PageHelp> = {
  ...
  'home.document': {
    ...
    features: [
      ...
      '插入整个资源时需选择实体访问地址（默认第一条）；按地址类型插入 PDF 摘页 / 图片 / 资源卡片。PDF 摘页可设起止页与纵向比例裁剪；块头/工具栏「划选裁剪」可在 PDF 上拖拽框选区域，松开后可「应用裁剪」或「发布笔记」（笔记写入页面 annotations 并随内容保存；以 fileId 锚定，PDF↔链接转换后重挂到新块；默认隐藏，块头「显示笔记」后按区域显示，开关随块持久化）；「高度 100%」按当前缩放将块高度调为刚好完整显示全部内容。',
